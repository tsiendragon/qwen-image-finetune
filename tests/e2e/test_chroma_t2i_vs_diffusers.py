"""
Tests comparing ChromaT2ITrainer against the official diffusers ChromaPipeline.
...
Validates that our trainer faithfully reproduces the pipeline for text-to-image:
  ...
  2. encode_prompt() produces matching T5 embeddings (with attention_mask)
  3. End-to-end inference with shared embeddings + fixed noise matches output

Key architectural details:
  - T5EncoderModel + T5TokenizerFast (google/t5-v1_1-xxl, max_length=512)
  - attention_mask (bool) passed to transformer
  - text_ids: all zeros (FLUX convention — not [0, i, i])
  - img_ids: [0, row, col]
  ...
  - guidance_scale default 0.0 (Chroma is distilled/classifier-free)

Tests use no pre-saved reference files — the official pipeline is the reference.

Usage:
    pytest tests/e2e/test_chroma_t2i_vs_diffusers.py -m e2e -v
"""
import logging
...
def t2i_trainer(infer_device):
    """ChromaT2ITrainer loaded from the same checkpoint (no LoRA)."""
    config = load_config_from_yaml(
        "tests/test_configs/test_example_chroma_t2i_fp16.yaml"
    )
    ...
    trainer = ChromaT2ITrainer(config)
...
def diffusers_pipeline(infer_device):
    """Official ChromaPipeline from the same checkpoint."""
...
class TestChromaT2IVsDiffusers:
    """Verify that ChromaT2ITrainer faithfully reproduces ChromaPipeline."""
    ...
    def test_prompt_embeddings_match_pipeline(self, t2i_trainer, diffusers_pipeline):
        """
        trainer.encode_prompt() must produce the same T5 embeddings and
        ...
        with torch.inference_mode():
            t_pe, t_mask, _ = trainer.encode_prompt(prompt=[PROMPT])
        ...
        assert_relative_error(t_pe, p_pe, rtol=1e-4, key="prompt_embeds (T5)")
