"""
Tests comparing SanaSprintT2ITrainer against the official diffusers SanaSprintPipeline.
...
Validates that our trainer faithfully reproduces the pipeline for text-to-image:
  ...
  2. encode_prompt() produces matching Gemma2 embeddings (select_index applied)
  3. End-to-end inference with shared embeddings + fixed noise matches output

Key architectural details:
  - Gemma2Model + GemmaTokenizerFast, max_length=300
  - Token selection: [0] + list(range(-299, 0)) — BOS + last 299 tokens
  - AutoencoderDC: encode returns .latent (not .latent_dist); decode returns .sample
  - SanaTransformer2DModel, 4D latents (B, C, H//32, W//32) — not packed
  - SCM timestep: scm_t = sin(t)/(cos(t)+sin(t))
  - Latent model input: (noisy/sigma_data) * sqrt(scm_t²+(1-scm_t)²)
  - guidance: fixed per-sample scalar tensor (distilled model)
  - DPMSolverMultistepScheduler

Tests use no pre-saved reference files — the official pipeline is the reference.

Usage:
    pytest tests/e2e/test_sana_sprint_t2i_vs_diffusers.py -m e2e -v
"""
import logging
...
def t2i_trainer(infer_device):
    """SanaSprintT2ITrainer loaded from the same checkpoint (no LoRA)."""
    config = load_config_from_yaml(
        "tests/test_configs/test_example_sana_sprint_t2i_fp16.yaml"
    )
    ...
    trainer = SanaSprintT2ITrainer(config)
...
def diffusers_pipeline(infer_device):
    """Official SanaSprintPipeline from the same checkpoint."""
...
class TestSanaSprintT2IVsDiffusers:
    """Verify that SanaSprintT2ITrainer faithfully reproduces SanaSprintPipeline."""
    ...
    def test_prompt_embeddings_match_pipeline(self, t2i_trainer, diffusers_pipeline):
        """
        trainer.encode_prompt() must produce the same Gemma2 embeddings and
        attention_mask as pipeline.encode_prompt() for the same prompt.
        """
        trainer = t2i_trainer
        pipe = diffusers_pipeline
        ...
        with torch.inference_mode():
            t_pe, t_mask = trainer.encode_prompt(prompt=[PROMPT])

        with torch.inference_mode():
            p_pe, p_mask = pipe.encode_prompt(
                prompt=PROMPT,
                device=device,
                batch_size=1,
            )

        assert_relative_error(t_pe, p_pe, rtol=1e-4, key="prompt_embeds (Gemma2)")
        assert torch.equal(t_mask.cpu(), p_mask.bool().cpu()), (
            "attention_mask mismatch between trainer and pipeline"
        )
    ...
    def test_end_to_end_output_matches_pipeline(self, t2i_trainer, diffusers_pipeline):
        ...
        batch = trainer.prepare_predict_batch_data(
            ...
            guidance_scale=trainer._guidance_scale,
        )
        embeddings = trainer.prepare_embeddings(batch, stage="predict")

        # ── Step B: shared initial noise latents (sigma_data-scaled) ────
