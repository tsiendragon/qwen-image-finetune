"""
Tests comparing OvisImageT2ITrainer against the official diffusers OvisImagePipeline.
...
Validates that our trainer faithfully reproduces the pipeline for text-to-image:
  ...
  2. encode_prompt() produces matching Qwen3 embeddings (chat template, drop 28 prefix tokens)
  3. End-to-end inference with shared embeddings + fixed noise matches output

Key architectural details:
  - Qwen3Model text encoder, Qwen2TokenizerFast
  - apply_chat_template(enable_thinking=False), drop first 28 tokens (system prefix)
  - last_hidden_state (not hidden_states[-3])
...
Usage:
    pytest tests/e2e/test_ovis_image_t2i_vs_diffusers.py -m e2e -v
"""
import logging
...
def t2i_trainer(infer_device):
    """OvisImageT2ITrainer loaded from the same checkpoint (no LoRA)."""
    config = load_config_from_yaml(
        "tests/test_configs/test_example_ovis_image_t2i_fp16.yaml"
    )
    ...
    trainer = OvisImageT2ITrainer(config)
...
def diffusers_pipeline(infer_device):
    """Official OvisImagePipeline from the same checkpoint."""
    pipe = OvisImagePipeline.from_pretrained(MODEL_ID, torch_dtype=torch.bfloat16)
...
class TestOvisImageT2IVsDiffusers:
    """Verify that OvisImageT2ITrainer faithfully reproduces OvisImagePipeline."""
    ...
    def test_prompt_embeddings_match_pipeline(self, t2i_trainer, diffusers_pipeline):
        """
        trainer.encode_prompt() must produce the same Qwen3 embeddings
        ...
        assert_relative_error(t_pe, p_pe, rtol=1e-4, key="prompt_embeds (Qwen3)")
