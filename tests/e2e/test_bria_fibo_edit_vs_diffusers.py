"""
Tests comparing BriaFiboEditTrainer against the official diffusers BriaFiboEditPipeline.

Validates that our trainer faithfully reproduces the IT2I edit pipeline:

  1. Component weights are identical (VAE, text_encoder, transformer)
  2. VAE encoding of source image matches pipeline (latent_dist.mean + mean/std norm)
  3. encode_prompt() produces matching embeddings
  4. End-to-end inference with shared embeddings + fixed noise matches output

Key architectural additions over T2I:
  - Source image encoded (deterministic: .latent_dist.mean) + normalized
  - Source img_ids have first dimension = 1 (vs target = 0)
  - Model input: cat([target_latents, source_latents], dim=1)
  - Output sliced to [:, :target_seq_len, :] — only target tokens
  - Source VAE encode: normalize with (latent - mean) * std

Tests use no pre-saved reference files — the official pipeline is the reference.

Usage:
    pytest tests/e2e/test_bria_fibo_edit_vs_diffusers.py -m e2e -v
"""
import logging
...
PROMPT = "Make the sky orange at sunset"
...
def edit_trainer(infer_device):
    config = load_config_from_yaml(
        "tests/test_configs/test_example_bria_fibo_edit_fp16.yaml"
    )
    ...
    trainer = BriaFiboEditTrainer(config)
...
def diffusers_pipeline(infer_device):
    pipe = BriaFiboEditPipeline.from_pretrained(MODEL_ID, torch_dtype=torch.bfloat16)
    pipe.to(infer_device)
    return pipe


@pytest.fixture(scope="module")
def source_image(infer_device):
    """Synthetic source image for testing."""
    torch.manual_seed(SEED)
    return torch.rand(1, 3, HEIGHT, WIDTH, device=infer_device, dtype=torch.bfloat16) * 2 - 1
...
class TestBriaFiboEditVsDiffusers:
    """Verify that BriaFiboEditTrainer faithfully reproduces BriaFiboEditPipeline."""
    ...
    # ── Test 2: VAE encoding of source image ─────────────────────────────

    @pytest.mark.e2e
    def test_vae_encoding_matches_pipeline(self, edit_trainer, diffusers_pipeline, source_image):
        """
        Trainer's source image VAE encode (latent_dist.mean + normalize) must match
        the pipeline's prepare_image_latents for the same input image.
        """
        trainer = edit_trainer
        pipe = diffusers_pipeline

        device = trainer.vae.device
        image = source_image.to(device, dtype=trainer.weight_dtype)

        # Trainer encoding
        with torch.inference_mode():
            t_latents = trainer._encode_source_image(image)  # normalized 4D

        # Pipeline encoding
        with torch.inference_mode():
            image_5d = image.unsqueeze(2)  # (B, C, 1, H, W)
            p_raw = pipe.vae.encode(image_5d).latent_dist.mean.squeeze(2)
            mean = torch.tensor(pipe.vae.config.latents_mean, device=device, dtype=torch.float32)
            std = torch.tensor(pipe.vae.config.latents_std, device=device, dtype=torch.float32)
            p_latents = ((p_raw.float() - mean.reshape(1, -1, 1, 1)) * std.reshape(1, -1, 1, 1)).to(trainer.weight_dtype)

        assert_relative_error(t_latents, p_latents, rtol=1e-4, key="source_vae_latents")
    ...
    def test_prompt_embeddings_match_pipeline(self, edit_trainer, diffusers_pipeline):
        ...
        with torch.inference_mode():
            p_pe, p_layers, p_mask = pipe.encode_prompt(
                prompt=PROMPT, device=device, batch_size=1,
            )

        assert_relative_error(t_pe, p_pe, rtol=1e-4, key="encoder_hidden_states")
        assert torch.equal(t_mask.cpu(), p_mask.bool().cpu()), "attention_mask mismatch"

    # ── Test 4: end-to-end ───────────────────────────────────────────────

    @pytest.mark.e2e
    def test_end_to_end_output_matches_pipeline(self, edit_trainer, diffusers_pipeline, source_image):
        trainer = edit_trainer
        pipe = diffusers_pipeline
        h, w = HEIGHT, WIDTH

        # Prepare batch with source image
        ...
        # Convert source_image to [0,1] for prepare_embeddings
        src_01 = (source_image.to(trainer.vae.device) / 2 + 0.5).clamp(0, 1)
        batch["control"] = src_01
        ...
        src_pil = PILImage.fromarray(
            (src_01.squeeze(0).permute(1, 2, 0).float().cpu().numpy() * 255).clip(0, 255).astype(np.uint8)
        )
