...
## HiDream-I1 T2I（`HiDreamI1T2ITrainer`）

新增模型支持，以下内容尚未验证。

架构特点：HiDreamImageTransformer2DModel（双流+单流块，MoE FFN），4 个文本编码器——CLIP×2（pooled，
投影拼接 → `pooled_prompt_embeds`）、T5（sequence → `encoder_hidden_states_t5`）、
Llama-3.1（所有 transformer 层堆叠 → `encoder_hidden_states_llama3`，形状 `(num_layers, B, seq, hidden)`），
`AutoencoderKL`（`vae_scale_factor`=2^(len(block_out_channels)-1)，decode: `lat/scaling_factor + shift_factor`），
4D latents `(B, 64, H//vae_scale, W//vae_scale)`（patchification 在 transformer 内部完成），
`FlowMatchEulerDiscreteScheduler` + dynamic shift，
**transformer 输出取反**：`noise_pred = -model_out`，训练目标 = `x_0 - noise`（反转符号）。

### 训练流程

| 场景 | 配置文件 | 状态 | 备注 |
|------|---------|------|------|
| LoRA + embedding cache (cache 阶段) | `hidream_i1_t2i_lora_with_cache.yaml --cache` | ⬜ 未测试 | 4 个编码器均需 GPU |
| LoRA + embedding cache (train 阶段) | `hidream_i1_t2i_lora_with_cache.yaml` | ⬜ 未测试 | |

### 推理 / Sampling

| 场景 | 状态 | 备注 |
|------|------|------|
| base model sampling (无 LoRA) | ⬜ 未测试 | `guidance_scale=5.0`，~50 步 |
| HiDream-I1 + LoRA sampling | ⬜ 未测试 | |
| CFG sampling (`guidance_scale > 1`) | ⬜ 未测试 | |
...
| 测试文件 | 测试内容 | 状态 |
|---------|---------|------|
| `tests/e2e/test_hidream_i1_t2i_vs_diffusers.py` | 与官方 `HiDreamImagePipeline` 对比：权重（6模块）、4编码器 embeddings（T5/Llama/CLIP×2）、端到端输出 | ⬜ 已创建，待 GPU 验证 |
| `tests/test_configs/test_example_hidream_i1_t2i_fp16.yaml` | HiDream-I1 测试配置文件 | ✅ 已创建 |
