...
### 修复
- **文档修正**: 更新了额外控制图像的命名规则描述，确保与实际代码实现一致
  - 修正了 `docs/huggingface-dataset.md` 中的控制图像命名规则，从 `<base>_1.*` 更新为 `<base>_control_1.*`
  - 更新了示例目录结构，确保与实际命名规则一致
  - 修正了 `src/data/dataset.py` 中的注释，确保与代码实现一致
  - 修正了 `docs/prd/multi_control.md` 中的额外控制图像命名描述
  - 修正了 `src/utils/hugginface.py` 中的控制图像收集逻辑描述

### 技术细节
- 额外控制图像的命名规则统一为 `<base>_control_1.*`、`<base>_control_2.*` 等格式
- 确保所有文档与代码实现保持一致，避免用户混淆
