...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        ... ).convert("RGB")
        >>> prompt = "Make Pikachu hold a sign that says 'Black Forest Labs is awesome', yarn art style,
...
class DreamOmni2Pipeline(
    ...
    def _get_t5_prompt_embeds(
        ...
        batch_size = len(prompt)  # type: ignore
        ...
        if ip_adapter_image_embeds is None:
            ...
            if len(ip_adapter_image) != self.transformer.encoder_hid_proj.num_ip_adapters:
                raise ValueError(
                    ...
                    f"Got {len(ip_adapter_image)} images and {self.transformer.encoder_hid_proj.num_ip_adapters}."
                )
            ...
            if len(ip_adapter_image_embeds) != self.transformer.encoder_hid_proj.num_ip_adapters:
                raise ValueError(
                    ...
                    f"{self.transformer.encoder_hid_proj.num_ip_adapters}."
                )
    ...
    def check_inputs(
        ...
        if prompt_embeds is not None and pooled_prompt_embeds is None:
            raise ValueError(
                "If `prompt_embeds` are provided, `pooled_prompt_embeds` also have to be passed. Make sure"
                "to generate `pooled_prompt_embeds` from the same text encoder that was used to generate"
                "`prompt_embeds`."
            )
        if negative_prompt_embeds is not None and negative_pooled_prompt_embeds is None:
            raise ValueError(
                "If `negative_prompt_embeds` are provided, `negative_pooled_prompt_embeds` also have to be passed."
                "Make sure to generate `negative_pooled_prompt_embeds` from the same text encoder that was used"
                "to generate `negative_prompt_embeds`."
    ...
    def __call__(
        self,
        images: list[PipelineImageInput] | None = None,
        prompt: Union[str, list[str]] = None,
        ...
        Args:
            image (`torch.Tensor`, `PIL.Image.Image`, `np.ndarray`, `List[torch.Tensor]`, `List[PIL.Image.Image]`,
                or `List[np.ndarray]`):
            ...
            joint_attention_kwargs (`dict`, *optional*):
                ...
                `self.processor` in
                [diffusers.models.attention_processor](https://github.com/huggingface/diffusers/blob/main/src/diffusers
        ...
        if height != original_height or width != original_width:
            logger.warning(
                f"Generation `height` and `width` have been adjusted to {height} and {width} to fit the"
                "model requirements."
            )
        ...
        if do_true_cfg:
            (
                ...
                prompt=negative_prompt,  # type: ignore
                prompt_2=negative_prompt_2,  # type: ignore
        ...
        if images is not None and not (
            isinstance(images[0], torch.Tensor) and images[0].size(1) == self.latent_channels
        ):
            tp_images = []
            for img in images:
                image = img
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if image_embeds is not None:
                    self._joint_attention_kwargs["ip_adapter_image_embeds"] = image_embeds  # type: ignore
                ...
                if do_true_cfg:
                    if negative_image_embeds is not None:
                        self._joint_attention_kwargs["ip_adapter_image_embeds"] = negative_image_embeds  # type: ignore
