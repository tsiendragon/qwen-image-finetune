...
def apply_rotary_emb_qwen(
    ...
    if use_real:
        ...
        assert isinstance(freqs_cis, torch.Tensor), "freqs_cis must be a tensor"
...
class QwenEmbedRope(nn.Module):
    ...
    def rope_params(self, index, dim, theta=10000) -> torch.Tensor:
        """
        Args:
    ...
    def forward(self, video_fhw, txt_seq_lens, device) -> tuple[torch.Tensor, torch.Tensor]:
        """
    ...
    def _compute_video_freqs(self, frame, height, width, idx=0) -> torch.Tensor:
...
if __name__ == "__main__":
    ...
    missing_keys, unexpected_keys = model.load_state_dict(flux_transformer.state_dict(), strict=False)
    print(missing_keys)
