...
def quantize_model_to_fp8(
    ...
    device: str = "cuda",
    quantize_config: Optional[Dict[str, Any]] = None,
    ...
    if engine.lower() == "te":
        quantized_model = _quantize_with_transformer_engine(
            model, config, skip_modules, device, verbose
        )
    elif engine.lower() == "bnb":
        quantized_model = _quantize_with_bitsandbytes(
            model, config, skip_modules, device, verbose
        )
    else:
...
def _quantize_with_transformer_engine(
    ...
    if gpu_capability[0] < 8:  # 需要计算能力8.0+（A100及以上）
        logger.warning(
            f"GPU compute capability {gpu_capability} may not fully support FP8. "
            f"Recommended: 8.0+ (A100/H100)"
        )
