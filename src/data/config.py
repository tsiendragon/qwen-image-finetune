...
class LoraConfig:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.lora_alpha, int) or self.lora_alpha <= 0:
            raise ValueError(f"lora_alpha must be a positive integer, got {self.lora_alpha}")

        if self.init_lora_weights not in ["gaussian", "normal", "zero"]:
            raise ValueError(f"init_lora_weights must be one of ['gaussian', 'normal', 'zero'], got {self.init_lora_weights}")

        if not isinstance(self.target_modules, list) or not self.target_modules:
            raise ValueError(f"target_modules must be a non-empty list, got {self.target_modules}")
...
class DataConfig:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.class_path, str) or not self.class_path:
            raise ValueError(f"class_path must be a non-empty string, got {self.class_path}")

        # 验证 init_args
        if not isinstance(self.init_args, dict):
            raise ValueError(f"init_args must be a dictionary, got {type(self.init_args)}")
...
class LoggingConfig:
    ...
    def __post_init__(self):
        ...
        if self.report_to not in ["tensorboard", "wandb", "all", "none"]:
            raise ValueError(f"report_to must be one of ['tensorboard', 'wandb', 'all', 'none'], got {self.report_to}")
...
class LRSchedulerConfig:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.warmup_steps, int) or self.warmup_steps < 0:
            raise ValueError(f"warmup_steps must be a non-negative integer, got {self.warmup_steps}")

        # 验证 num_cycles
        if not isinstance(self.num_cycles, (int, float)) or self.num_cycles <= 0:
            raise ValueError(f"num_cycles must be a positive number, got {self.num_cycles}")
    ...
    def __post_init__(self):
        ...
        if "weight_decay" in self.init_args:
            ...
            if not isinstance(wd, (int, float)) or wd < 0:
                raise ValueError(f"weight_decay in init_args must be non-negative, got {wd}")

        # 验证 betas（如果存在）
        if "betas" in self.init_args:
            betas = self.init_args["betas"]
            if not isinstance(betas, (list, tuple)) or len(betas) != 2:
                raise ValueError(f"betas in init_args must be a list/tuple of 2 elements, got {betas}")
...
class TrainConfig:
    ...
    def __post_init__(self):
        ...
        if self.checkpoints_total_limit is not None:
            if not isinstance(self.checkpoints_total_limit, int) or self.checkpoints_total_limit <= 0:
                raise ValueError(f"checkpoints_total_limit must be a positive integer or None, got {self.checkpoints_total_limit}")
        ...
        if self.mixed_precision not in ["fp16", "bf16", "no"]:
            raise ValueError(f"mixed_precision must be one of ['fp16', 'bf16', 'no'], got {self.mixed_precision}")
